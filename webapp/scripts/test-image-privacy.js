...
const { validateStatement, clarifyStatement, parseStatementAnswer, readStatementImage, parseStatementImage } = require("../statement-reader");
...
async function main() {
  ...
  assert.equal(parseStatementAnswer("2025或2026年"), null);
  for (const currency of ["NTD", "NT$", "台幣", "twd"]) {
    assert.equal(validateStatement({ complete: true, warnings: [], rows: [{ ...row, currency }] }).entries[0].amount, 230);
  }
  assert.ok(validateStatement({ complete: true, warnings: ["missing_year", "missing_currency"], rows: merchantRows }).clarification);
  assert.equal(validateStatement({ complete: true, warnings: ["cropped_rows"], rows: merchantRows }).clarification, null);
