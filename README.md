...
ETF / 股票資料庫透過 `fetch()` 載入，正式測試請使用本地伺服器，不建議直接用 `file://` 開啟。本地 server 會提供 `/api/update-database`，網頁開啟後會嘗試背景更新官方資料；若更新失敗，App 會沿用最後通過驗證的快照。完整報告與諮詢訂金改用 LINE Pay 導轉式金流；前端只負責導轉，真正解鎖以後端確認交易金額、幣別、訂單與交易編號的結果為準。未設定 LINE Pay 商店憑證時，付款按鈕會安全停用且不建立訂單。
...
LINE Pay sandbox 測試部署前可先執行：

```powershell
node webapp\scripts\validate-payment-config.js --sandbox
```
...
sandbox 與 production 都需要 LINE Pay 商店提供的 `Channel ID`、`Channel Secret`。production 另外要求公開前後端 HTTPS 網址、完整報告 NT$499、諮詢訂金 NT$200、諮詢費 NT$1,500，以及 IG / LINE 諮詢連結。商店申請尚未核准時可維持無憑證狀態，健康檢查會回報 `configured: false`，前端不會建立訂單或收款。真實金鑰只放 Render 環境變數，不提交到 Git。

GitHub Pages runtime 設定使用 repository variables：`CONSULTATION_IG_URL`、`CONSULTATION_LINE_URL`、`FULL_REPORT_PRICE_TWD`、`CONSULTATION_DEPOSIT_TWD`、`CONSULTATION_FEE_TWD`。Render 後臺另外使用 `LINE_PAY_ENV`、`LINE_PAY_CHANNEL_ID`、`LINE_PAY_CHANNEL_SECRET` 保存 LINE Pay 設定；`ECPAY_LEGACY_CALLBACK_ENABLED` 預設維持 `0`。

LINE 官方帳號整合需要在 Render 後臺設定 `LINE_CHANNEL_SECRET` 與 `LINE_CHANNEL_ACCESS_TOKEN`。LINE Login channel 必須建立在同一個 Provider，另設 `LINE_LOGIN_CHANNEL_ID`、`LINE_LOGIN_CHANNEL_SECRET`、`LINE_LOGIN_CALLBACK_URL`、`LINE_LIFF_ID` 與 `LINE_LIFF_URL`。所有 secret 只放 Render 環境變數，不提交到 Git。

正式對客戶開放前，需在 LINE Developers Console 公開 Provider page，並在隱私權政策與服務說明中告知 Messaging API 與 LINE Login 由同一服務提供者共同使用會員識別與財務資料。

會員版先直接使用 Render 網址，讓 App 與 `/api` 同源。LINE Login callback 設為 `https://small-budget-cashflow-api.onrender.com/api/auth/line/callback`；LIFF Endpoint URL 設為 `https://small-budget-cashflow-api.onrender.com/`。登入成功後以 30 天 `Secure`、`HttpOnly`、`SameSite=Lax` cookie 保存 session，瀏覽器 JavaScript 不保存 LINE token。

會員登入 API：

- `GET /api/auth/line/start`、`GET /api/auth/line/callback`、`POST /api/auth/line/liff`
- `GET /api/auth/session`、`POST /api/auth/logout`、`POST /api/auth/logout-all`
- `GET /api/users/me/bootstrap`、`POST /api/users/me/onboarding/complete`
- `DELETE /api/users/me/account`，必須附帶 `X-Confirm-Delete: DELETE MY ACCOUNT`
...
- `買 0056 10000`、`ETF 00878 5000`：記為投資買入，從剩餘現金流扣除並累積 ETF 部位。
- `0056 配息 800`、`賣出 0056 收到 12000`：記為投資流入，不混入薪資等一般收入；賣出款不會猜測成本或自動扣除 ETF 部位。
- `投資ETF` 或缺少金額時不會猜測記帳，會追問買入、配息或賣出及金額。
- `昨天付房租 1.2萬`、`繳保險 3000`、`繳房貸 2萬`：分別同步日期與固定支出分類。
- `查明細`、`修改第2筆 180`、`刪除第3筆`：查詢或修正本月紀錄。
- `刪除晚餐`、`取消今天的晚餐`：搜尋最近 90 天符合用途、分類或標的的記帳。只有一筆時直接顯示確認；多筆時先選編號，再按「確認刪除」。找不到、選擇模糊、候選過期或帳目已變更時都不會刪除。
